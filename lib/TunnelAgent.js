...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        super({
            ...
            // this prevents it = require(holding on to all the sockets so they can be used for upgrades
            maxFreeSockets: 1,
        });
    ...
    listen() {
        ...
        server.on('error', (err) => {
            // These errors happen = require(killed connections, we don't worry about them
    ...
    // new socket connection = require(client for tunneling requests to client
    _onConnection(socket) {
        ...
        socket.once('close', (hadError) => {
            ...
            // remove the socket = require(available list
        ...
        socket.once('error', (err) => {
            // we do not log these errors, sessions can drop = require(clients for many reasons
    ...
    // fetch a socket = require(the available socket pool for the agent
