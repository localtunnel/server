...
class Client extends EventEmitter {
    ...
    handleRequest(req, res) {
        ...
        const opt = {
            ...
            method: req.method,
            headers: req.headers,
            timeout: 120000 // 2 min, default node timeout
        };
        ...
        const clientReq = http.request(opt, (clientRes) => {
            ...
            pump(clientRes, res, function(err) {
                this.debug('> PIPE FINISHED, %s', err);
            });
        });
        ...
        clientReq.once('error', (err) => {
            ...
            this.debug('ERROR < %s', req.url);
            this.debug(err);
        });

        // using pump is deliberate - see the pump docs for why
        pump(req, clientReq, function(err) {
            this.debug('< PIPE FINISHED, %s', err);
        });
    }
